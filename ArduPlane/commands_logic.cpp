...
#if AP_EMERGENCYDESCENT_ENABLED
/*
  Emergency descent: single-point behaviour.

  MAV_CMD_NAV_EMERGENCY_DESCENT_TARGET is the only command that matters here.
  Reaching it starts a dive straight from the aircraft's current position onto
  the commanded location -- there is no entry gate and no alignment phase.
  (An earlier revision flew to a separate entry point and orbited there until
  the run-in bearing lined up; that loiter is how a fixed-wing corrects
  heading, so on release the aircraft could be anywhere on the circle, not
  travelling through the gate -- observed as a wide circle near the entry
  point that missed the intended line. Diving straight from wherever the
  aircraft already is removes that failure mode: the terminal law is
  proportional navigation, which is precisely the control law for converging
  onto a target from an arbitrary starting position and heading.)

  MAV_CMD_NAV_EMERGENCY_DESCENT_ENTRY is no longer given special handling; if
...
bool Plane::do_emergency_descent(const AP_Mission::Mission_Command& cmd)
{
    ...
    char reason[32];
    if (!g2.emergency_descent.start(cmd.content.location, current_loc, reason, sizeof(reason))) {

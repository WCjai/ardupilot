...
const AP_Param::GroupInfo AP_EmergencyDescent::var_info[] = {
    ...
    // @Description: Command pitch as a body ROTATION RATE toward the line-of-sight-derived target flight-path angle (the same target the ordinary vertical law tracks -- shallow far out, steepening as range closes) instead of converging on it as an angle target -- the same rate-control mechanism ACRO mode uses. Angle-based attitude control (the ordinary vertical law, and EMG_DIVE_PITCH above) converges through the normal fixed-wing attitude controller, which was found empirically to have an unexplained achieved-pitch ceiling well short of its own commanded target, and which in any case becomes unreliable approaching +-90 deg (Euler angles are singular there). Body-rate control has neither problem. The target angle is capped at EMG_VDIVE_PITCH as the steepest allowed -- it does not dive to a fixed angle regardless of geometry, which would miss the target entirely (no altitude/time budget left for lateral correction once diving near-vertically). Self-regulating -- no separate duration parameter, it simply commands less rate as the target angle is approached. Enabled by default: measured better than the angle-based path in every geometry tried. Against a target 970 m out from 400 m it impacted 0.0 m away in 67 s, where the angle-based path was capped near -20 deg by the vehicle's attitude controller and impacted 41 m away in 92 s; against a close target it reaches near-vertical where the angle-based path cannot.
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
    AP_GROUPINFO("VDIVE_EN", 18, AP_EmergencyDescent, _vdive_enable, 1),
